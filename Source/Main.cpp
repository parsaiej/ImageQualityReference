...
{
    // Re-enumerate the display modes in case of monitor change.
    ...
    // Handle a case where a display change may cause an out-of-range refresh rate.
    gDXGIDisplayRefreshRatesIndex = std::min(gDXGIDisplayRefreshRatesIndex, static_cast<int>(gDXGIDisplayRefreshRates.size() - 1));
